...
$typ=$row["Typ"]; 
	...
	if($typ=="Singlechoice"){
		...
		while($row=mysqli_fetch_array($exec))
		{
			if($Trainer=='externes_feedback')
			{
				$sql2 = "SELECT COUNT(case when ".$Frage." = '|".$row["Answers"]."|' then 1 else null end) AS result FROM (SELECT ".$Frage." FROM externes_feedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 23:59:59'AND Leistung LIKE '".$Leistung."' ORDER BY Datum DESC LIMIT 10) AS a";
			}
		
			else{
				$sql2 = "SELECT COUNT(case when ".$Frage." = '|".$row["Answers"]."|' then 1 else null end) AS result FROM (SELECT ".$Frage." FROM externes_feedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 23:59:59'AND Leistung LIKE '".$Leistung."' AND Username = '".$Trainer."' ORDER BY Datum DESC LIMIT 10) AS a";
			}
		$exec2 = mysqli_query($link, $sql2);
		$row2=mysqli_fetch_array($exec2);
