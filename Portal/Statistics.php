...
$typ=$row["Typ"]; 
	...
	$sql_get_Anzahl_abgegebenes_feedback = "SELECT COUNT(".$Frage.") FROM kursfeedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 00:00:00'AND Leistung LIKE '".$Leistung."' AND ".$Frage." != 'NULL' AND Username LIKE '".$Trainer."'";
	...
	if($typ=="Bewertung"){
		...
		{	
			$sql2 = "SELECT COUNT(".$Frage.") FROM kursfeedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 00:00:00'AND Leistung LIKE '".$Leistung."' AND ".$Frage." LIKE '%|".$row["Answers"]."|%' AND Username LIKE '".$Trainer."'";
	...
	if($typ=="Bewertung"){
		...
		while($row=mysqli_fetch_array($exec))
		{
			$sql2 = "SELECT COUNT(case when ".$Frage." = '|".$row["Answers"]."|' then 1 else null end) AS result FROM (SELECT ".$Frage." FROM kursfeedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 00:00:00'AND Leistung LIKE '".$Leistung."' AND Username LIKE '".$Trainer."' AND ".$Frage." != 'NULL'ORDER BY Datum DESC LIMIT 10) AS a";
			$exec2 = mysqli_query($link, $sql2);
			$row2=mysqli_fetch_array($exec2);
	...
	$query = "SELECT ROUND(AVG(".$Frage."),1) FROM (SELECT * FROM kursfeedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 00:00:00'AND Leistung LIKE '".$Leistung."' AND Username LIKE '".$Trainer."' AND ".$Frage." != 'NULL' ORDER BY Datum DESC LIMIT 10) AS a";
	$exec = mysqli_query($link,$query);
