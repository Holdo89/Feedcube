	...
	if($row["Typ"]=="Bewertung" || $row["Typ"]=="Multiplechoice")
	{
		...
		$i=1; //Bewertungsmöglichkeiten 1,...,6
 		...
 		while($i<=intval($rows["Anzahl_Antworten"])){
			...
			$query = "SELECT COUNT(".$Frage.") FROM umfragenfeedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 00:00:00' AND ".$Frage." LIKE '%|".$rows_answers["Answers"]."|%'";
			$exec = mysqli_query($link,$query);
	...
	else if($row["Typ"]=="Schieberegler")
 	{
		...
		$rowy = mysqli_fetch_array($exec);
 		while($i<=$rowy["columns"]){
			$query = "SELECT COUNT(".$Frage.") FROM umfragenfeedback WHERE Datum <= '".$datum_min." 23:59:59' AND Datum >= '".$datum_max." 00:00:00'AND ".$Frage." < ".$feedback_range[$i]." AND ".$Frage." >= ".$feedback_range[$i-1]."";
			$exec = mysqli_query($link,$query);
			while($row = mysqli_fetch_array($exec))
			{
				echo $row["COUNT(".$Frage.")"].",";
