...
{
    $sql = "SELECT * FROM bewertung_answers WHERE Fragenspezifisch = ".$ID." AND Intern_".$ID." = 1 OR Intern_".$ID." = 0 ORDER BY post_order_no ASC";
}
...
{
	echo'<p><input type="hidden" value="'.$row["Answers"].'_unchecked" name="checkbox[]"><input type="checkbox" style="margin-left:0px;" id="'.$row["Answers"].'_'.$Typ.'_'.$Fragentyp.'" name="checkbox[]" value="'.$row["Answers"].'" ';
    if($row["Intern_".$ID]==1)
    {
        echo'checked';
    }
    echo'><label for="'.$row["Answers"].'_'.$Typ.'_'.$Fragentyp.'" style="border:none;"> '.$row["Answers"].'</label>';
