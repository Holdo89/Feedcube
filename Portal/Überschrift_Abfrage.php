...
while($row_überschrift = mysqli_fetch_assoc($result_überschrift)) {
    ...
    <button type="button" id="element" style="width:250px; background-color:'; $sql='SELECT farbe FROM system'; $exec=mysqli_query($link,$sql); $result=mysqli_fetch_assoc($exec); echo $result['farbe']; echo'" onclick = "display(undefined,\'extern\',undefined)"><i class="fa fa-question" style="font-size:19px" aria-hidden="true"></i> Frage hinzufügen</button>
    </div>';
}
echo'</div>';
?>
