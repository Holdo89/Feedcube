...
require_once "session.php";
    ...
    if($Trainer=='externes_feedback')
    {
        ...
        {
            $sql_date_last = "SELECT Datum, Trainer, Leistung FROM externes_feedback ORDER BY Datum DESC";
    ...
    else
    {
        ...
        if($Feedback_abgegeben)
        {
            $sql_date_last = "SELECT Datum, Trainer, Leistung FROM externes_feedback WHERE Username = '".$Trainer."' ORDER BY Datum DESC";
            $query_date_last = mysqli_query($link,$sql_date_last);
            $row_date_last = mysqli_fetch_array($query_date_last);
