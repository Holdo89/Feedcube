...
{
	echo'<div style="background-color:ghostwhite; margin-bottom:10px; padding: 5px;"><input type="hidden" value="'.$row["Answers"].'_unchecked" name="checkbox[]"><input type="checkbox" style="margin-left:0px;" id="'.$row["ID"].'_'.$Typ.'_'.$Fragentyp.'" name="checkbox[]" value="'.$row["Answers"].'" ';
    if($row["Frage_".$ID]==1)
    {
        echo'checked';
    }
    echo"><input type='hidden' value='".$row["ID"]."_unchecked_englisch' name='checkboxenglisch[]'>
    <input type='checkbox' style='margin-left:0px; display:none' id='".$row["ID"]."_extern_Multiplechoice_englisch' name='checkboxenglisch[]' value='".$row["Answers_Englisch"]."' onclick='return false' checked>
    <label id='label_".$row["ID"]."' for='".$row["ID"]."_".$Typ."_".$Fragentyp."' style='border:none;'> ".$row["Answers"]."</label><button type='button' style='border:none; background:white; margin-left:10px;' onclick='ShowFragenspezifischeDetails(".$row["ID"].",\"".$Fragentyp."\")'><i class='fa fa-pencil'></i></button><button type='button' style='border:none; background:white; margin-left:10px;' onclick='Antwort_löschen(".$row["ID"].",\"".$Fragentyp."\")'><i class='fa fa-trash'></i></button>";
