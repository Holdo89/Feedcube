...
<script>
    ...
    current_chapter_new = 0;
    var current
    var text = "";
    var abgegebenes_feedback_array = [];
    var current_html = [];
    var Anzahl_Fragen = <?php echo $Anzahl_Fragen; ?>; //gesamte Anzahl der Fragen
    var Überschrift_array = [<?php foreach ($Überschrift_array as $Überschrift) {
                                    echo "'" . $Überschrift . "',";
                                } ?>];
    var Fragen_array = [<?php foreach ($Fragen_array as $Frage) {
                            echo "'" . $Frage . "',";
                        } ?>];
    var Fragentyp_array = [<?php foreach ($Fragentyp_array as $Fragentyp) {
                                echo "'" . $Fragentyp . "',";
                            } ?>];
    var Default_Schieberegler_array = [<?php foreach ($Default_Schieberegler_array as $Default_Schieberegler) {
                                            echo "'" . $Default_Schieberegler . "',";
                                        } ?>];
    var Max_Schieberegler_array = [<?php foreach ($Max_Schieberegler_array as $Max_Schieberegler) {
                                        echo "'" . $Max_Schieberegler . "',";
                                    } ?>];
    var Min_Schieberegler_array = [<?php foreach ($Min_Schieberegler_array as $Min_Schieberegler) {
                                        echo "'" . $Min_Schieberegler . "',";
                                    } ?>];
    get_next_question();
    var i = 0;
    ...
    function push_all_Answers() {
        ...
        if (current_chapter == current_chapter_new) {
            while (i < Fragenzahl) {
                try {
            ...
            var checked = false;
            var test = 0;
            var differentradiogroups=1;
            var container = document.getElementById(current_chapter-1);
            var allchecked = container.querySelectorAll("input[type=radio]:checked");
            var allunchecked = container.querySelectorAll("input[type=radio]");
            
            try{
                while(test<allunchecked.length)
                {
                    if(allunchecked[test].name!=allunchecked[test+1].name)
                    {
                        differentradiogroups++;
                    }
                    test++
                }
            }
            ...
            if(allchecked.length==differentradiogroups || allunchecked.length==0)
            {
                checked = true;
            }
            ...
            if (checked == false) {
                ...
                document.getElementById(current_chapter).style.display = "block";
                document.getElementById(current_chapter - 1).style.display = "none";
            }
        }
    }
    ...
    function get_next_question() {
        ...
        do {
            ...
            if (Fragentyp_array[Fragenzahl] == "Bewertung") {
                ...
                var Bewertung_array_value = [<?php foreach ($Bewertung_array_value as $Bewertung) {
                                                    echo "[";
                                                    foreach ($Bewertung as $Single) {
                                                        echo "'" . $Single . "',";
                                                    }
                                                    echo "],";
                                                } ?>];
                var Bewertung_array = [<?php foreach ($Bewertung_array as $Bewertung_show) {
                                            echo "[";
                                            foreach ($Bewertung_show as $Single_show) {
                                                echo "'" . $Single_show . "',";
                                            }
                                            echo "],";
                                        } ?>];
                ...
                var Multiplechoice_array_value = [<?php foreach ($Multiplechoice_array_value as $Multiplechoice) {
                                                        echo "[";
                                                        foreach ($Multiplechoice as $Multiple) {
                                                            echo "'" . $Multiple . "',";
                                                        }
                                                        echo "],";
                                                    } ?>];
                var Multiplechoice_array = [<?php foreach ($Multiplechoice_array as $Multiplechoice_show) {
                                                echo "[";
                                                foreach ($Multiplechoice_show as $Multiple_show) {
                                                    echo "'" . $Multiple_show . "',";
                                                }
                                                echo "],";
                                            } ?>];
            ...
            if (Überschrift_array[Fragenzahl - 1] != Überschrift_array[Fragenzahl]) {
                //weiter und zurück button hinzufügen
                document.getElementById("Fragenzahl").value = Fragenzahl;
                if (current_chapter_new > 0) {
                    ...
                    document.getElementById("container").innerHTML = document.getElementById("container").innerHTML + <?php echo "\"<input class='center_button' type='button' id ='button' value='" . $Buttontext . "' onclick='push_all_Answers()' style='display:inline; width:40%'><label class='label_progress' id='label_progress'>Überschrift \""; ?> + parseInt(current_chapter_new + 1) + <?php echo "\" von " . $Anzahl_Überschrift . "</label><progress class='progressbar' id='progress' value='\""; ?> + parseInt(current_chapter_new + 1) + <?php echo "\"' max='" . $Anzahl_Überschrift . "'></progress></form>\"" ?>;
                } else
                    document.getElementById("container").innerHTML = document.getElementById("container").innerHTML + <?php echo "\"<input class='center_button' type='button' id ='button' value='" . $Buttontext . "' onclick='push_all_Answers()'><label class='label_progress' id='label_progress'>Überschrift \""; ?> + parseInt(current_chapter_new + 1) + <?php echo "\" von " . $Anzahl_Überschrift . "</label><progress class='progressbar' id='progress' value='\""; ?> + parseInt(current_chapter_new + 1) + <?php echo "\"' max='" . $Anzahl_Überschrift . "'></progress></form>\"" ?>;
                ...
                console.log("Current Chapter new" + current_chapter_new)
                if (current_chapter == current_chapter_new) {
                    text = text + "<div id='" + current_chapter_new + "'><div class='chapter'>" + Überschrift_array[Fragenzahl - 1] + "</div>" + document.getElementById("container").innerHTML + "</div>";
                } else {
                    text = text + "<div id='" + current_chapter_new + "' style='display:none'><div class='chapter'>" + Überschrift_array[Fragenzahl - 1] + "</div>" + document.getElementById("container").innerHTML + "</div>";
                }
                current_chapter_new = current_chapter_new + 1;
                document.getElementById("container").innerHTML = "";
            }
        } while (Fragenzahl < Anzahl_Fragen)
